...
{
	$forum_head['style_base'] = '<link rel="stylesheet" type="text/css" href="'.$base_url.'/style/base.css" />';
...
if ($forum_user['g_read_board'] && $forum_user['g_search'])
{
	...
	if (!$forum_user['is_guest'])
		$visit_links['posts'] = '<span id="visit-recent"'.(empty($visit_links) ? ' class="first-item"' : '').'><a href="'.forum_link($forum_url['search_user_posts'], $forum_user['id']).'" title="'.$lang_common['My posts title'].'">'.$lang_common['My posts'].'</a></span>';
}
