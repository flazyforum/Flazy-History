...
{
	$forum_head['rss'] = '<link rel="alternate" type="application/rss+xml" href="'.forum_link($forum_url['feed_forum'], array('rss', $id, $cur_forum['sort_by'] == '1' ? 'posted' : 'last_post')).'" title="RSS" />';
	$forum_head['atom'] = '<link rel="alternate" type="application/atom+xml" href="'.forum_link($forum_url['feed_forum'], array('atom', $id, $cur_forum['sort_by'] == '1' ? 'posted' : 'last_post')).'" title="ATOM" />';
}
...
if ($forum_user['is_guest'])
	$visit_elements['<!-- forum_welcome -->'] = '<p id="welcome"><span>'.$lang_common['Not logged in'].'</span> <span>'.sprintf($lang_common['Login nag'], '<a href="'.forum_link($forum_url['login']).'">'.$lang_common['Login nag 2'].'</a>' , '<a href="'.forum_link($forum_url['register']).'">'.$lang_common['Login nag 3'].'</a>').'</span></p>';
else
...
$main_elements['<!-- forum_main_head -->'] =  '<div class="main-head">'."\n\t".'<h1 class="hn"><span>'.((isset($forum_page['main_head'])) ? $forum_page['main_head'] : forum_htmlencode(is_array($last_crumb = end($forum_page['crumbs'])) ? reset($last_crumb) : $last_crumb)).(isset($forum_page['main_head_pages']) ? ' <small>'.$forum_page['main_head_pages'].'</small>' : '').'</span></h1>'."\n".'</div>';
