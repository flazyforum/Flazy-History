...
$forum_page['main_options'] = array(
	'new_search'	=> '<span'.(empty($forum_page['main_options']) ? ' class="item1"' : '').'><a class="mod-option-search" href="'.forum_link($forum_url['users']).'">'.$lang_ul['Perform new search'].'</a></span>'
);
...
$forum_page['frm-info'] = array(
	'wildcard'	=> '<li><span>'.$lang_ul['Wildcard info'].'</span></li>',
	'group'		=> '<li><span>'.$lang_ul['Group info'].'</span></li>',
	'sort'		=> '<li><span>'.$lang_ul['Sort info'].'</span></li>'
);
...
<?php ($hook = get_hook('ul_pre_username')) ? eval($hook) : null; if ($forum_user['g_search_users']): ?>
				<div class="sf-set set<?php echo ++$forum_page['item_count'] ?>">
					<div class="sf-box text">
						...
						<span class="fld-input"><input type="text" id="fld<?php echo $forum_page['fld_count'] ?>" name="username" value="<?php echo forum_htmlencode($forum_page['username']) ?>" size="35" maxlength="25" class="inputbox" /></span>
					</div>
				</div>
<?php endif; ?>
...
// Grab the users
$query = array(
	'SELECT'	=> 'u.id, u.username, u.title, u.num_posts, u.registered, u.last_visit, u.rep_plus, u.rep_minus, g.g_id, g.g_user_title',
	'FROM'		=> 'users AS u',
	'JOINS'		=> array(
		array(
...
if ($forum_db->num_rows($result))
{
	...
	if ($forum_config['o_rep_enabled'] && $forum_user['g_rep_enable'] && $forum_user['rep_enable'])
		$forum_page['table_header']['reputation'] = '<th class="tc'.count($forum_page['table_header']).'" scope="col">'.$lang_ul['Reputation'].'</th>';
	...
	$forum_page['table_header']['last visit'] = '<th class="tc'.count($forum_page['table_header']).'" scope="col">'.$lang_ul['Last visit'].'</th>';
