...
<?php ($hook = get_hook('st_fl_about_onlinetoday')) ? eval($hook) : null; ?>
...
<?php ($hook = get_hook('st_fl_about_topauthor')) ? eval($hook) : null; ?>
			<div class="ct-set data-set set<?php echo ++$forum_page['item_count'] ?>">
				<div class="ct-box data-box">
...
<?php ($hook = get_hook('st_fl_about_topreplies')) ? eval($hook) : null; ?>
			<div class="ct-set data-set set<?php echo ++$forum_page['item_count'] ?>">
				<div class="ct-box data-box">
...
$forum_page['main_menu'] = array(
	'online_today'	=> '<li'.(($section == 'onlinetoday')  ? ' class="active"' : '').'><a href="'.forum_link($forum_url['statistic'], 'onlinetoday').'"><span>'.$lang_stat['Online today'].'</span></a></li>',
	'top_author'	=> '<li'.(($section == 'topauthor')  ? ' class="active"' : '').'><a href="'.forum_link($forum_url['statistic'], 'topauthor').'"><span>'.$lang_stat['Top author little'].'</span></a></li>',
	'top_replies'	=> '<li'.(($section == 'topreplies') ? ' class="active"' : '').'><a href="'.forum_link($forum_url['statistic'], 'topreplies').'"><span>'.$lang_stat['Top replies little'].'</span></a></li>',
	'top_views'		=> '<li'.(($section == 'topviews') ? ' class="active"' : '').'><a href="'.forum_link($forum_url['statistic'], 'topviews').'"><span>'.$lang_stat['Top views little'].'</span></a></li>',
...
if ($section == 'onlinetoday')
{
	...
	($hook = get_hook('st_fl_pre_onlinetoday_header_load')) ? eval($hook) : null;
...
<?php
	...
	else
	{
		...
		$forum_page['table_header']['username'] = '<th class="tc'.count($forum_page['table_header']).'" style="width:75%" scope="col">'.$lang_common['Last visit'].'</th>';
	$forum_page['table_header']['username'] = '<th class="tc'.count($forum_page['table_header']).'" style="width:25%" scope="col">'.$lang_common['Username'].'</th>';
	$forum_page['table_header']['last_visit'] = '<th class="tc'.count($forum_page['table_header']).'" style="width:75%" scope="col">'.$lang_stat['Last visit'].'</th>';
		...
		($hook = get_hook('st_fl_onlinetoday_results_pre_header_output')) ? eval($hook) : null;
?>
		<div class="ct-group">
			<table cellspacing="0">
			<thead>
...
<?php
		...
		while ($online_today = $forum_db->fetch_assoc($result)) 
		{
			...
			$forum_page['table_row']['username'] = '<td class="tc'.count($forum_page['table_row']).'"><a href="'.forum_link($forum_url['user'], $online_today['id']).'">'.forum_htmlencode($online_today['username']).'</a></td>';
			$forum_page['table_row']['last_visit'] = '<td class="tc'.count($forum_page['table_row']).'">'.format_time($online_today['last_visit']).$lang_common['Title separator'].flazy_format_time($online_today['last_visit']).'</td>';

			++$forum_page['item_count'];

			($hook = get_hook('st_fl_onlinetoday_results_row_pre_data_output')) ? eval($hook) : null;
...
else if ($section == 'topauthor')
{
	...
	($hook = get_hook('st_fl_pre_topauthor_header_load')) ? eval($hook) : null;
	...
	($hook = get_hook('st_fl_topauthor_results_pre_header_output')) ? eval($hook) : null;
...
<?php
	...
	($hook = get_hook('st_fl_top_author_qr')) ? eval($hook) : null;
	$result = $forum_db->query_build($query) or error(__FILE__, __LINE__);
	...
	while ($top_author = $forum_db->fetch_assoc($result))
	{
		...
		$forum_page['table_row'] = array();
		$forum_page['table_row']['user'] = '<td class="tc'.count($forum_page['table_row']).'"><a href="'.forum_link($forum_url['user'], $top_author['id']).'">'.forum_htmlencode($top_author['username']).'</a></td>';
		$forum_page['table_row']['registered'] = '<td class="tc'.count($forum_page['table_row']).'">'.format_time($top_author['registered']).'</td>';
		$forum_page['table_row']['num_posts'] = '<td class="tc'.count($forum_page['table_row']).'">'.forum_number_format($top_author['num_posts']).'</td>';
		...
		($hook = get_hook('st_fl_topauthor_results_row_pre_data_output')) ? eval($hook) : null;
...
<?php
	...
	($hook = get_hook('st_fl_top_author_end')) ? eval($hook) : null;
...
else if ($section == 'topreplies')
{
	...
	($hook = get_hook('st_fl_top_replies_qr')) ? eval($hook) : null;
	$result = $forum_db->query_build($query) or error(__FILE__, __LINE__);

	($hook = get_hook('st_fl_pre_top_replies_header_load')) ? eval($hook) : null;
...
<?php
	...
	{
		$forum_page['table_header'] = array();
		$forum_page['table_header']['username'] = '<th class="tc'.count($forum_page['table_header']).'" style="width:40%" scope="col">'.$lang_common['Username'].'</th>';
		$forum_page['table_header']['author'] = '<th class="tc'.count($forum_page['table_header']).'" style="width:20%" scope="col">'.$lang_stat['Author'].'</th>';
		$forum_page['table_header']['replies'] = '<th class="tc'.count($forum_page['table_header']).'" style="width:20%" scope="col">'.$lang_stat['Replies'].'</th>';
		$forum_page['table_header']['views'] = '<th class="tc'.count($forum_page['table_header']).'" style="width:20%" scope="col">'.$lang_stat['Views'].'</th>';

		($hook = get_hook('st_fl_topreplies_results_pre_header_output')) ? eval($hook) : null;
...
<?php
		...
		while ($top_replies = $forum_db->fetch_assoc($result))
		{
			...
			$forum_page['table_row'] = array();
			$forum_page['table_row']['topic'] = '<td class="tc'.count($forum_page['table_row']).'"><a href="'.forum_link($forum_url['topic'], $top_replies['id']).'">'.forum_htmlencode($top_replies['subject']).'</a></td>';
			$forum_page['table_row']['user'] = '<td class="tc'.count($forum_page['table_row']).'"><a href="'.forum_link($forum_url['user'], $top_replies['poster_id']).'">'.forum_htmlencode($top_replies['poster']).'</a></td>';
			$forum_page['table_row']['num_replies'] = '<td class="tc'.count($forum_page['table_row']).'">'.forum_number_format($top_replies['num_replies']).'</td>';
			$forum_page['table_row']['num_views'] = '<td class="tc'.count($forum_page['table_row']).'">'.forum_number_format($top_replies['num_views']).'</td>';

			++$forum_page['item_count'];

			($hook = get_hook('st_fl_topreplies_results_row_pre_data_output')) ? eval($hook) : null;
...
<?php
	...
	($hook = get_hook('st_fl_top_replies_end')) ? eval($hook) : null;
...
else if ($section == 'topviews')
{
	...
	($hook = get_hook('st_fl_top_views_qr')) ? eval($hook) : null;
	$result = $forum_db->query_build($query) or error(__FILE__, __LINE__);

	($hook = get_hook('st_fl_pre_top_views_header_load')) ? eval($hook) : null;
...
<?php
	...
	if ($forum_db->num_rows($result))
	{
		...
		($hook = get_hook('st_fl_topviews_results_pre_header_output')) ? eval($hook) : null;
...
<?php
		...
		while ($top_views = $forum_db->fetch_assoc($result))
		{
			...
			$forum_page['table_row'] = array();
			$forum_page['table_row']['topic'] = '<td class="tc'.count($forum_page['table_row']).'"><a href="'.forum_link($forum_url['topic'], $top_views['id']).'">'.forum_htmlencode($top_views['subject']).'</a></td>';
			$forum_page['table_row']['user'] = '<td class="tc'.count($forum_page['table_row']).'"><a href="'.forum_link($forum_url['user'], $top_views['poster_id']).'">'.forum_htmlencode($top_views['poster']).'</a></td>';
			$forum_page['table_row']['num_views'] = '<td class="tc'.count($forum_page['table_row']).'">'.forum_number_format($top_views['num_views']).'</td>';
			$forum_page['table_row']['num_replies'] = '<td class="tc'.count($forum_page['table_row']).'">'.forum_number_format($top_views['num_replies']).'</td>';

			++$forum_page['item_count'];

			($hook = get_hook('st_fl_topviews_results_row_pre_data_output')) ? eval($hook) : null;
?>
				<tr class="<?php echo ($forum_page['item_count'] % 2 != 0) ? 'odd' : 'even' ?><?php echo ($forum_page['item_count'] == 1) ? ' row1' : '' ?>">
					<?php echo implode("\n\t\t\t\t\t\t", $forum_page['table_row'])."\n" ?>
				</tr>
...
<?php
	...
	($hook = get_hook('st_fl_top_views_end')) ? eval($hook) : null;
