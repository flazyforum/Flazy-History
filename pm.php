...
$forum_page['main_menu']['about'] = '<li class="item1"><a href="'.forum_link($forum_url['profile'], array($forum_user['id'], 'about')).'#brd-crumbs-top"><span>'.$lang_profile['Section about'].'</span></a></li>';
...
$forum_links['pm'] = array(
 	'<li'.($section == 'inbox' ? ' class="active item1"' : ' class="normal"').'><a href="'.forum_link($forum_url['pm'], 'inbox').'#brd-crumbs-top">'.$lang_pm['Inbox'].'</a></li>',
 	'<li'.($section == 'outbox' ? ' class="active item1"' : ' class="normal"').'><a href="'.forum_link($forum_url['pm'], 'outbox').'#brd-crumbs-top">'.$lang_pm['Outbox'].'</a></li>',
 	'<li'.($section == 'write' ? ' class="active item1"' : ' class="normal"').'><a href="'.forum_link($forum_url['pm'], 'write').'#brd-crumbs-top">'.$lang_pm['Compose message'].'</a></li>'
);
...
<?php
	...
	else
	{
		...
		$forum_page['table_header']['select'] = '<th class="tc'.count($forum_page['table_header']).'" scope="col"><input onclick="return select_all(this.checked);" type="checkbox" name="delete_all" value="" /></th>';
		$forum_page['table_header']['icons'] = '<th class="tc'.count($forum_page['table_header']).'" scope="col"><img src="'.$icons_path.'/p_sent.png" height="16" width="16" alt="Status" title="Status"/></th>';
...
<?php
		...
		while ($cur_pm = $forum_db->fetch_assoc($result))
		{
			...
			$forum_page['table_row']['delete'] = '<td class="tc'.count($forum_page['table_row']).'"><input type="checkbox" name="id[]" value="'.$cur_pm['id'].'" /></td>';
			$forum_page['table_row']['icons'] = '<td class="tc'.count($forum_page['table_row']).'"><img src="'.$icons_path.'/'.($cur_pm['status'] == 'delivered' ? 'p_'.$section.'_' : 'p_').''.$cur_pm['status'].'.png" height="16" width="16" alt="'.$lang_pm['Status '.$cur_pm['status']].'" title="'.$lang_pm['Status '.$cur_pm['status']].'" /></td>';
...
?>
			...
			<div class="frm-buttons">
				<span class="submit"><input type="submit" name="delete" value="<?php echo $lang_pm['Delete selected'] ?>" onclick="return confirm_delete();"/></span>
			</div>
		</form>
	</div>
	...
	function select_all(all_checked)
	{
		var a = document.all && !window.opera ? document.all : document.getElementsByTagName("*");

		for (var i = a.length; i--;)
		{
			if (a[i].tagName.toLowerCase() == 'input' && a[i].getAttribute("type") == "checkbox" && a[i].getAttribute("name") == "id[]")
...
?>
				<div class="sf-set set<?php echo ++$forum_page['item_count'] ?>">
					<div class="sf-box text required longtext">
						...
						<span class="fld-input"><input id="fld<?php echo $forum_page['fld_count'] ?>" type="text" name="req_receiver" value="<?php if (isset($_POST['req_receiver']) || isset($receiver)) echo forum_htmlencode($receiver); ?>" size="80" maxlength="255" class="inputbox" /></span>
					</div>
				</div>
<?php
...
?>
				<div class="sf-set set<?php echo ++$forum_page['item_count'] ?>">
					<div class="sf-box text required longtext">
						...
						<span class="fld-input"><input id="fld<?php echo $forum_page['fld_count'] ?>" type="text" name="subject" value="<?php if (isset($_POST['subject']) || isset($subject)) echo forum_htmlencode($subject); ?>" size="80" maxlength="255" class="inputbox" /></span>
					</div>
				</div>
...
<?php require FORUM_ROOT.'bb.php'; ?>
						<div class="txt-input"><span class="fld-input"><textarea id="text" class="inputbox" name="req_message" rows="10" cols="65"><?php if (isset($_POST['req_message']) || isset($message)) echo forum_htmlencode($message); ?></textarea></span></div>
					</div>
				</div>
			</fieldset>
			<div class="frm-buttons">
...
else if ($section == 'message')
{
	...
	if ((($cur_message['email_setting'] == '0' && !$forum_user['is_guest']) || $forum_user['is_admmod']) && $forum_user['g_send_email'])
		$forum_page['post_contacts']['email'] = '<span class="user-email'.(empty($forum_page['post_contacts']) ? ' item1' : '').'"><a href="mailto:'.forum_htmlencode($cur_message['email']).'">'.$lang_profile['E-mail'].'<span>&#160;'.forum_htmlencode($cur_message['username']).'</span></a></span>';
	else if ($cur_message['email_setting'] == '1' && !$forum_user['is_guest'] && $forum_user['g_send_email'])
		$forum_page['post_contacts']['email'] = '<span class="user-email'.(empty($forum_page['post_contacts']) ? ' item1' : '').'"><a href="'.forum_link($forum_url['email'], $cur_message['sender_id']).'">'.$lang_topic['E-mail'].'<span>&#160;'.forum_htmlencode($cur_message['username']).'</span></a></span>';
	...
	$forum_page['post_actions']['delete'] = '<span class="delete-topic'.(empty($forum_page['post_actions']) ? ' item1' : '').'"><a href="'.forum_link($forum_url['pm_delete'], $cur_message['id']).'" onclick="return confirm(\''.$lang_pm['Delete confirmation 1'].'\');" >'.$lang_pm['Delete message'].'</a></span>';

	if ($type == 'outbox' && ($cur_message['status'] == 'draft' || $cur_message['status'] == 'sent'))
		$forum_page['post_actions']['edit'] = '<span class="edit-topic'.(empty($forum_page['post_actions']) ? ' item1' : '').'"><a href="'.forum_link($forum_url['pm_edit'], $cur_message['id']).'">'.$lang_pm['Edit message'].'</a></span>';
...
<?php ($hook = get_hook('pm_fl_message_pre_subject')) ? eval($hook) : null; ?>
				<div class="sf-set set<?php echo ++$forum_page['item_count'] ?>">
					<div class="sf-box text required longtext">
						<label for="fld<?php echo ++$forum_page['fld_count'] ?>"><span><?php echo $lang_pm['Subject'] ?></span></label><br />
						<span class="fld-input"><input id="fld<?php echo $forum_page['fld_count'] ?>" type="text" name="subject" value="<?php echo forum_htmlencode($cur_message['subject']) ?>" size="80" maxlength="255" class="inputbox" /></span>
					</div>
				</div>
<?php ($hook = get_hook('pm_fl_message_pre_req_message')) ? eval($hook) : null; ?>
				<div class="txt-set set<?php echo ++$forum_page['item_count'] ?>">
					<div class="txt-box textarea required">
						<label for="fld<?php echo ++$forum_page['fld_count'] ?>"><span><?php echo $lang_pm['Message'] ?> <em><?php echo $lang_common['Required'] ?></em></span></label>
<?php require FORUM_ROOT.'bb.php'; ?>
						<div class="txt-input"><span class="fld-input"><textarea id="text" class="inputbox" name="req_message" rows="10" cols="65"></textarea></span></div>
					</div>
				</div>
			</fieldset>
			<div class="frm-buttons">
