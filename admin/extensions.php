...
if (isset($_GET['install']) || isset($_GET['install_hotfix']))
{
	...
	$forum_page['crumbs'] = array(
		...
		array($lang_admin_common['Extensions'], forum_link('admin/extensions.php?section=manage')),
		array((strpos($id, 'hotfix_') === 0) ? $lang_admin_common['Manage hotfixes'] : $lang_admin_common['Manage extensions'], (strpos($id, 'hotfix_') === 0) ? forum_link('admin/extensions.php?section=hotfixes') : forum_link('admin/extensions.php?section=manage')),
...
?>
			...
			<p><a href="<?php echo forum_link('admin/extensions.php?section=manage') ?>"><?php echo $lang_admin_common['Manage extensions'] ?></a></p>
		</div>
	</div>
<?php
		...
		else
		{
			...
			if (strpos($id, 'hotfix_') === 0)
				redirect(forum_link('admin/extensions.php?section=hotfixes'), $lang_admin_ext['Hotfix installed'].' '.$lang_admin_common['Redirect']);
			else
				redirect(forum_link('admin/extensions.php?section=manage'), $lang_admin_ext['Extension installed'].' '.$lang_admin_common['Redirect']);
		}
	}
...
<?php
	...
	if (!isset($ext_data['extension']['flazy']) )
		$form_warnings[] = '<li>'.$lang_admin_ext['Punbb extension warning'].'</li>';
...
else if (isset($_GET['uninstall']))
{
	...
	if (isset($_POST['uninstall_cancel']))
		redirect(forum_link('admin/extensions.php?section=manage'), $lang_admin_common['Cancel redirect']);
...
<?php
		...
		else
		{
			...
			if (strpos($id, 'hotfix_') === 0)
				redirect(forum_link('admin/extensions.php?section=hotfixes'), $lang_admin_ext['Hotfix uninstalled'].' '.$lang_admin_common['Redirect']);
			else
				redirect(forum_link('admin/extensions.php?section=manage'), $lang_admin_ext['Extension uninstalled'].' '.$lang_admin_common['Redirect']);
		}
	}
...
else if (isset($_GET['flip']))
{
	...
	if ($section == 'hotfixes')
		redirect(forum_link('admin/extensions.php?section=hotfixes'), ($disable ? $lang_admin_ext['Hotfix disabled'] : $lang_admin_ext['Hotfix enabled']).' '.$lang_admin_common['Redirect']);
	else
		redirect(forum_link('admin/extensions.php?section=manage'), ($disable ? $lang_admin_ext['Extension disabled'] : $lang_admin_ext['Extension enabled']).' '.$lang_admin_common['Redirect']);
}
...
if ($section == 'hotfixes')
{
	...
	$forum_page['crumbs'] = array(
		...
		array($lang_admin_common['Forum administration'], forum_link('admin/admin.php')),
		array($lang_admin_common['Extensions'], forum_link('admin/extensions.php?section=manage')),
		array($lang_admin_common['Manage hotfixes'], forum_link('admin/extensions.php?section=hotfixes'))
	);
...
<?php
	...
	foreach ($inst_exts as $id => $ext)
	{
		...
		$forum_page['ext_actions'] = array(
			...
			'upgrade'	=> '<span><a href="'.$base_url.'/admin/extensions.php?section=hotfixes&amp;install_hotfix='.$id.'">'.$lang_admin_ext['Upgrade hotfix'].'</a></span>'
		);
...
else if ($section == 'manage')
{
	...
	$forum_page['crumbs'] = array(
		...
		array($lang_admin_common['Forum administration'], forum_link('admin/admin.php')),
		array($lang_admin_common['Extensions'], forum_link('admin/extensions.php?section=manage')),
		array($lang_admin_common['Manage extensions'], forum_link('admin/extensions.php?section=manage'))
	);
...
<?php
	...
	foreach ($inst_exts as $id => $ext)
	{
		...
		$forum_page['ext_actions'] = array(
			...
			'upgrade'	=> '<span><a href="'.$base_url.'/admin/extensions.php?section=manage&amp;install='.$id.'">'.$lang_admin_ext['Upgrade extension'].'</a></span>'
		);
...
else if ($section == 'repository')
{
	...
	$forum_page['crumbs'] = array(
		array($forum_config['o_board_title'], forum_link($forum_url['index'])),
		array($lang_admin_common['Forum administration'], forum_link('admin/admin.php')),
		array($lang_admin_common['Extensions'], forum_link('admin/extensions.php?section=repository')),
		array($lang_admin_common['Manage repository'], forum_link('admin/extensions.php?section=repository'))
	);

	($hook = get_hook('aex_section_repository_pre_header_load')) ? eval($hook) : null;

	define('FORUM_PAGE_SECTION', 'extensions');
	define('FORUM_PAGE', 'admin-extensions-repository');
	require FORUM_ROOT.'header.php';

	// START SUBST - <forum_main>
	ob_start();

	($hook = get_hook('aex_section_rep_install_output_start')) ? eval($hook) : null;

?>
	<div class="main-subhead">
		<h2 class="hn"><span><?php echo $lang_admin_ext['Rep extensions available'] ?></span></h2>
	</div>
	<div class="main-content main-frm main-extensions">
<?php
	...
	// Loop through any available repository
	if (isset($forum_repository['extension']))
	{
		// If there's only one repository, add one layer of arrays so we can foreach over it
		if (!is_array(current($forum_repository['extension'])))
			$forum_repository['extension'] = array($forum_repository['extension']);

		foreach ($forum_repository['extension'] as $extension)
		{
			if (!array_key_exists($extension['attributes']['id'], $inst_exts))
			{
				$forum_page['ext_item'][] = '<div class="ct-box info-box hotfix available">'."\n\t\t\t".'<h3 class="ct-legend hn"><span>'.forum_htmlencode($extension['content']).'</span></h3>'."\n\t\t\t".'<ul>'."\n\t\t\t\t".'<li><span>'.sprintf($lang_admin_ext['Extension by'], 'Flazy').'</span></li>'."\n\t\t\t\t".'<li><span>'.$lang_admin_ext['Repository description'].'</span></li>'."\n\t\t\t".'</ul>'."\n\t\t\t\t".'<p class="options"><span class="first-item"><a href="'.$base_url.'/admin/extensions.php?install_hotfix='.urlencode($extension['attributes']['id']).'">'.$lang_admin_ext['Install extension'].'</a></span></p>'."\n\t\t".'</div>';
				++$num_exts;
			}
		}
	}

	($hook = get_hook('aex_section_repository_pre_display_available_ext_list')) ? eval($hook) : null;
...
<?php
	...
	($hook = get_hook('aex_section_repository_pre_display_installed_ext_list')) ? eval($hook) : null;
...
<?php
	...
	foreach ($inst_exts as $id => $ext)
	{
		...
		$forum_page['ext_actions'] = array(
			'flip'		=> '<span class="first-item"><a href="'.$base_url.'/admin/extensions.php?section=repository&amp;flip='.$id.'&amp;csrf_token='.generate_form_token('flip'.$id).'">'.($ext['disabled'] != '1' ? $lang_admin_ext['Disable'] : $lang_admin_ext['Enable']).'</a></span>',
			'uninstall'	=> '<span><a href="'.$base_url.'/admin/extensions.php?section=repositorye&amp;uninstall='.$id.'">'.$lang_admin_ext['Uninstall'].'</a></span>',
			'upgrade'	=> '<span><a href="'.$base_url.'/admin/extensions.php?section=manage&amp;install='.$id.'">'.$lang_admin_ext['Upgrade extension'].'</a></span>'
		);
...
<?php
	...
	($hook = get_hook('aex_section_repository_end')) ? eval($hook) : null;
