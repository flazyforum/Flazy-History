...
if (isset($_POST['add_forum']))
{
	...
	redirect(forum_link('admin/forums.php'), $lang_admin_forums['Forum added'].' '.$lang_admin_common['Redirect']);
}
...
else if (isset($_GET['del_forum']))
{
	...
	if (isset($_POST['del_forum_cancel']))
		redirect(forum_link('admin/forums.php'), $lang_admin_common['Cancel redirect']);
	...
	else	// If the user hasn't confirmed the delete
	{
		...
		$forum_page['crumbs'] = array(
			...
			array($lang_admin_common['Forums'], forum_link('admin/forums.php')),
...
?>
	...
	<div class="main-content main-frm">
		<form class="frm-form" method="post" accept-charset="utf-8" action="<?php echo forum_link('admin/forums.php') ?>?del_forum=<?php echo $forum_to_delete ?>">
			<div class="hidden">
				<input type="hidden" name="csrf_token" value="<?php echo generate_form_token(forum_link('admin/forums.php').'?del_forum='.$forum_to_delete) ?>" />
			</div>
...
else if (isset($_POST['update_positions']))
{
	...
	redirect(forum_link('admin/forums.php'), $lang_admin_forums['Forums updated'].' '.$lang_admin_common['Redirect']);
}
...
else if (isset($_GET['edit_forum']))
{
	...
	if (isset($_POST['save']))
	{
		...
		if (isset($_POST['read_forum_old']))
		{
			...
			while ($cur_group = $forum_db->fetch_assoc($result))
			{
				...
				$perms_new = array(
					'read_forum'	=>	($cur_group['g_read_board']) ? isset($_POST['read_forum_new'][$cur_group['g_id']]) ? '1' : '0' : intval($_POST['read_forum_old'][$cur_group['g_id']]),
		...
		redirect(forum_link('admin/forums.php'), $lang_admin_forums['Forum updated'].' '.$lang_admin_common['Redirect']);
	}
	else if (isset($_POST['revert_perms']))
	{
		...
		redirect(forum_link('admin/forums.php').'?edit_forum='.$forum_id, $lang_admin_forums['Permissions reverted'].' '.$lang_admin_common['Redirect']);
	}
	...
	$forum_page['form_info']['groups'] = '<li><span>'. sprintf($lang_admin_forums['Forum perms groups info'], '<a href="'.forum_link('admin/groups.php').'">'.$lang_admin_forums['User groups'].'</a>').'</span></li>';
...
?>
	...
	<div class="main-content main-frm">
		<form method="post" class="frm-form" accept-charset="utf-8" action="<?php echo forum_link('admin/forums.php') ?>?edit_forum=<?php echo $forum_id ?>">
			<div class="hidden">
...
<?php ($hook = get_hook('afo_edit_forum_pre_forum_name')) ? eval($hook) : null; ?>
				<div class="sf-set set<?php echo ++$forum_page['item_count'] ?>">
					<div class="sf-box text">
						...
						<span class="fld-input"><input type="text" id="fld<?php echo $forum_page['fld_count'] ?>" name="forum_name" size="35" maxlength="80" value="<?php echo forum_htmlencode($cur_forum['forum_name']) ?>" class="inputbox" /></span>
					</div>
				</div>
<?php ($hook = get_hook('afo_edit_forum_pre_forum_descrip')) ? eval($hook) : null; ?>
				<div class="txt-set set<?php echo ++$forum_page['item_count'] ?>">
					<div class="txt-box textarea">
						...
						<div class="txt-input"><span class="fld-input"><textarea id="fld<?php echo $forum_page['fld_count'] ?>" class="inputbox" name="forum_desc" rows="3" cols="50"><?php echo forum_htmlencode($cur_forum['forum_desc']) ?></textarea></span></div>
					</div>
				</div>
				...
				<div class="sf-set set<?php echo ++$forum_page['item_count'] ?>">
					<div class="sf-box select">
						...
						<span class="fld-input"><select id="fld<?php echo $forum_page['fld_count'] ?>" name="sort_by">
							...
							<option value="2"<?php if ($cur_forum['sort_by'] == '2') echo ' selected="selected"' ?>><?php echo $lang_admin_forums['Sort alphabet'] ?></option>
<?php ($hook = get_hook('afo_edit_forum_modify_sort_by')) ? eval($hook) : null; ?>
						</select></span>
					</div>
				</div>
<?php ($hook = get_hook('afo_edit_forum_pre_forum_redirect_url')) ? eval($hook) : null; ?>
				<div class="sf-set set<?php echo ++$forum_page['item_count'] ?>">
					<div class="sf-box text">
						...
						<span class="fld-input"><?php echo ($cur_forum['num_topics']) ? '<input type="text" id="fld'.$forum_page['fld_count'].'" name="redirect_url" size="45" maxlength="100" value="'.$lang_admin_forums['Only for empty forums'].'" disabled="disabled" class="inputbox" />' : '<input type="text" id="fld'.$forum_page['fld_count'].'" name="redirect_url" size="45" maxlength="100" value="'.forum_htmlencode($cur_forum['redirect_url']).'" class="inputbox" />'; ?></span>
					</div>
				</div>
...
?>
	...
	<div class="main-content main-frm">
		<form class="frm-form" method="post" accept-charset="utf-8" action="<?php echo forum_link('admin/forums.php') ?>?action=adddel">
			<div class="hidden">
				<input type="hidden" name="csrf_token" value="<?php echo generate_form_token(forum_link('admin/forums.php').'?action=adddel') ?>" />
			</div>
...
<?php ($hook = get_hook('afo_pre_new_forum_name')) ? eval($hook) : null; ?>
				<div class="sf-set set<?php echo ++$forum_page['item_count'] ?>">
					<div class="sf-box text">
						...
						<span class="fld-input"><input type="text" id="fld<?php echo $forum_page['fld_count'] ?>" name="forum_name" size="35" maxlength="80" class="inputbox" /></span>
					</div>
				</div>
...
?>
	...
	<div class="main-content main-frm">
		<form class="frm-form" method="post" accept-charset="utf-8" action="<?php echo forum_link('admin/forums.php') ?>?action=edit">
			<div class="hidden">
				<input type="hidden" name="csrf_token" value="<?php echo generate_form_token(forum_link('admin/forums.php').'?action=edit') ?>" />
			</div>
...
?>
				<fieldset class="mf-set set<?php echo ++$forum_page['item_count'] ?><?php echo ($forum_page['item_count'] == 1) ? ' mf-head' : ' mf-extra' ?>">
					<legend><span><?php printf($lang_admin_forums['Edit or delete'], '<a href="'.forum_link('admin/forums.php').'?edit_forum='.$cur_forum['fid'].'">'.$lang_admin_forums['Edit'].'</a>', '<a href="'.forum_link('admin/forums.php').'?del_forum='.$cur_forum['fid'].'">'.$lang_admin_forums['Delete'].'</a>') ?></span></legend>
					<div class="mf-box">
...
<?php ($hook = get_hook('afo_pre_edit_cur_forum_position')) ? eval($hook) : null; ?>
						<div class="mf-field">
							<label for="fld<?php echo ++$forum_page['fld_count'] ?>"><span><?php echo $lang_admin_forums['Position label'] ?></span></label><br />
							<span class="fld-input"><input type="text" id="fld<?php echo $forum_page['fld_count'] ?>" name="position[<?php echo $cur_forum['fid'] ?>]" size="3" maxlength="3" value="<?php echo $cur_forum['disp_position'] ?>" class="inputbox" /></span>
						</div>
					</div>
