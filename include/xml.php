...
function xml_to_array($raw_xml)
{
	...
	foreach ($vals as $xml_elem)
	{
		...
		{
			$php_stmt_main = ($x_type == 'open') ? $php_stmt.'[$x_type]'.$add.'[\'content\'] = $xml_elem[\'value\'];' : $php_stmt.'[$x_tag]'.$add.' = $xml_elem[\'value\'];';
...
function validate_manifest($xml_array, $folder_name)
{
	...
	else
	{
		...
		if (isset($ext['flazy']) && isset($ext['minversion']) && version_compare(clean_version($forum_config['o_cur_version']), clean_version($ext['minversion']), '<'))
