...
?>
					<div class="sf-set set<?php echo ++$forum_page['item_count'] ?>">
						<div class="sf-box text">
							...
							<span class="fld-input"><input id="fld<?php echo $forum_page['fld_count'] ?>" type="text" class="inputbox" name="answer[]" size="80" maxlength="70" value="<?php echo ($answers != '' && isset($answers[$opt_num]) ? forum_htmlencode(forum_trim($answers[$opt_num])) : '') ?>" /></span>
						</div>
					</div>
<?php
...
<?php $forum_page['group_count'] = $forum_page['item_count'] = $forum_page['fld_count'] = 0; ?>
				<fieldset class="frm-group group<?php echo ++$forum_page['group_count'] ?>">
<?php ($hook = get_hook('fn_fl_pre_poll_enable_read')) ? eval($hook) : null;
	if ($forum_config['p_poll_enable_read']): ?>
					<div class="sf-set set<?php echo ++$forum_page['item_count'] ?>">
...
<?php endif;
($hook = get_hook('fn_fl_pre_poll_allow_day')) ? eval($hook) : null; ?>
					<div class="sf-set set<?php echo ++$forum_page['item_count'] ?>">
						<div class="sf-box text">
							...
							<span class="fld-input"><input id="fld<?php echo $forum_page['fld_count'] ?>" type="text" class="inputbox" name="days" size="5" maxlength="5" value="<?php echo ($days != '') ? $days : '0'; ?>" /></span>
						</div>
					</div>
<?php ($hook = get_hook('fn_fl_pre_poll_max_votes')) ? eval($hook) : null; ?>
					<div class="sf-set set<?php echo ++$forum_page['item_count'] ?>">
						<div class="sf-box text">
							...
							<span class="fld-input"><input id="fld<?php echo $forum_page['fld_count'] ?>" type="text" class="inputbox" name="votes" size="5" maxlength="5" value="<?php echo ($votes != '') ? $votes : '0'; ?>" /></span>
						</div>
					</div>
				</fieldset>
			</div>
