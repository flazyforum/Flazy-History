...
function create_search_cache($keywords, $author, $search_in = false, $forum = array(-1), $show_as = 'topics', $sort_by = null, $sort_dir = 'DESC')
{
	...
	// Flood protection
	if ($forum_user['last_search'] && ($now - $forum_user['last_search']) < $forum_user['g_search_flood'] && ($now - $forum_user['last_search']) >= 0)
	...
	if (!in_array(-1, $forum) || (!$forum_config['o_search_all_forums'] && !$forum_user['is_admmod']))
