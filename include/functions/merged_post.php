...
function merged_post($post_info, &$new_pid)
{
	...
	$return = ($hook = get_hook('fn_add_merged_start')) ? eval($hook) : null;
	if ($return != null)
		return;
	...
	($hook = get_hook('fn_merged_qr_update_post')) ? eval($hook) : null;
	...
	($hook = get_hook('fn_merged_qr_update_topics')) ? eval($hook) : null;
	$forum_db->query_build($query) or error(__FILE__, __LINE__);
	...
	($hook = get_hook('fn_merged_qr_update_forums')) ? eval($hook) : null;
	$forum_db->query_build($query) or error(__FILE__, __LINE__);

	// Update user last post
	$query = array(
		'UPDATE'	=> 'users',
		'SET'		=> 'last_post='.$post_info['posted'].', user_agent=\''.$post_info['user_agent'].'\'',
		'WHERE'		=> 'id='.$post_info['poster_id']
	);
	($hook = get_hook('fn_merged_qr_update_users')) ? eval($hook) : null;
	$forum_db->query_build($query) or error(__FILE__, __LINE__);

	$new_pid = $post_info['post_id'];

	($hook = get_hook('fn_merged_post_end')) ? eval($hook) : null;
}
