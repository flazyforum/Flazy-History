...
// They deserve all the credit for writing it. I made small modifications for it to suit Forum and it's coding standards (http://www.phpbb.com).
function server_parse($socket, $expected_response)
{
	...
	while (substr($server_response, 3, 1) != ' ')
	{
		if (!($server_response = fgets($socket, 256)))
			error('Не удалось получить кодировку почтового сервера. Пожалуйста, свяжитесь с администратором форума.', __FILE__, __LINE__);
	}

	if (!(substr($server_response, 0, 3) == $expected_response))
		error('Невозможно отправить e-mail. Пожалуйста, свяжитесь с администратором форума и сообщите следующую информацию об ошибке на SMTP-сервере: "'.$server_response.'"', __FILE__, __LINE__);
}
